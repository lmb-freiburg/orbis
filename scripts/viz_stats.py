...
def log_and_visualize_calib_stats(
    calib_stats_path="results/calib_stats_combined.pt", # combined. detailed or semantic
    output_dir="results/calib_logs"
):
    """
    Loads calib_stats.pt (multi-head format), prints tabular summaries, 
    and saves multi-panel heatmap plots for each head separately.
    """
    ...
    # Extract available heads and timesteps dynamically
    heads = list(calib_stats.keys())
    
    for head in heads:
        t_grid = sorted(list(calib_stats[head].keys()))
        sample_count = calib_stats[head][t_grid[0]].get("n", "N/A")

        # 2. Console Summary Logging per Head
        print("\n" + "=" * 65)
        print(f"   HEAD: {head.upper()} (Total Samples n = {sample_count})")
        print("=" * 65)
        print(f"{'Timestep (t)':<14} | {'Mean (Min)':<10} | {'Mean (Max)':<10} | {'Global Mean':<12} | {'Global Std':<10}")
        print("-" * 65)

        for t_val in t_grid:
            mu_map = calib_stats[head][t_val]["mean"]
            std_map = calib_stats[head][t_val]["std"]
        ...
        for i, t_val in enumerate(t_grid):
            mu_np = calib_stats[head][t_val]["mean"].cpu().numpy()
            std_np = calib_stats[head][t_val]["std"].cpu().numpy()
        ...
        fig.suptitle(
            f"Calibration Maps ({head.capitalize()} Head, n={sample_count} clips)",
            fontsize=14,
            y=0.98,
        )
        
        save_path = os.path.join(output_dir, f"calibration_heatmaps_{head}.png")
        fig.savefig(save_path, dpi=200, bbox_inches="tight")
        plt.close(fig)

        print(f"Heatmap visualization for '{head}' saved to: {save_path}")


if __name__ == "__main__":
    log_and_visualize_calib_stats(
        calib_stats_path="results/calib_stats_combined.pt",
        output_dir="results/calib_logs"
    )