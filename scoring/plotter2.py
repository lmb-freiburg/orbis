...
def normalize_map_with_calib(raw_map, t_val, calib_stats, eps=1e-8):
    """raw_map: [H, W] tensor for one sample at one t. Uses population (calib) stats, not self-stats."""
    mean = calib_stats[t_val]["mean"]
    std = calib_stats[t_val]["std"]
    return (raw_map - mean) / (std + eps)
