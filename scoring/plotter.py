...
def plot_error_maps_from_disk(sample_id, sample_label, target_frame_folder, t_grid,
    ...
    # Figure: 2 rows (detail, semantic) x len(t_grid) cols, raw maps
    fig1, axes1 = plt.subplots(2, len(t_grid), figsize=(3 * len(t_grid), 6))
    for row, stream in enumerate(["detail", "semantic"]):
        for i, t_val in enumerate(t_grid):
            im = axes1[row, i].imshow(error_maps[stream][t_val], cmap='hot')
            axes1[row, i].set_title(f"{stream} t={t_val}")
            axes1[row, i].axis('off')
            plt.colorbar(im, ax=axes1[row, i], fraction=0.046)
    fig1.suptitle(f"{sample_label} — raw 16x16 error maps (detail vs semantic)")
    fig1.tight_layout()
    fig1.savefig(f"{RESULTS_DIR}/error_map_raw_{sample_label.lower()}.png", dpi=150)

    # Figure: overlay on target frame, same 2x N layout
    ...
    for row, stream in enumerate(["detail", "semantic"]):
        for i, t_val in enumerate(t_grid):
            ...
            axes2[row, i].imshow(target_frame)
            axes2[row, i].imshow(err_up, cmap='hot', alpha=0.5)
            axes2[row, i].set_title(f"{stream} t={t_val}")
            axes2[row, i].axis('off')
    fig2.suptitle(f"{sample_label} — error overlay (detail vs semantic)")
    fig2.tight_layout()
    fig2.savefig(f"{RESULTS_DIR}/error_map_overlay_{sample_label.lower()}.png", dpi=150)
